...
- **@.claude/steering/project-context.md** - プロジェクトコンテキスト（目的、要件、制約条件）
- **@.claude/steering/technical-spec.md** - 技術仕様（使用ライブラリ、フレームワーク、ツールチェーン）
...
- **@.claude/steering/typescript-testing.md** - テストルール（TDD手法、テスト構造、アサーション方針）
- **@.claude/steering/ui-design-integration.md** - UIデザイン統合ルール（Figmaキャッシュ読み取り、ui-fixer統合）
...
**例外**: ビジュアル検証タスク実行時のみ、visual-checkerを呼び出し可能（詳細は@.claude/steering/sub-agents.md参照）
```
...
1. **タスク実行**
   - `specs/stories/{NOTION_STORY_ID}-{title}/tasks/` からタスクファイルを読み込み実行
   - タスクの「メタ情報」に記載された依存成果物を確認
   - 完了条件をすべて満たす

2. **進捗管理（タスクファイルのみ更新）**
   - タスクファイル（specs/stories/{NOTION_STORY_ID}-{title}/tasks/*.md）内のチェックボックスを更新
...
`specs/stories/{NOTION_STORY_ID}-{title}/tasks/task-*.md` パターンのファイルから、未完了のチェックボックス `[ ]` が残っているものを選択して実行
...
例: `specs/stories/{NOTION_STORY_ID}-{title}/tasks/analysis/調査結果.md`、`specs/stories/{NOTION_STORY_ID}-{title}/tasks/analysis/api-spec.md`
...
```json
{
  ...
  "nextAction": {
    ...
    "prompt": "タスクファイル: specs/stories/{NOTION_STORY_ID}-{title}/tasks/[タスクファイル名] で変更されたUIコンポーネント（[コンポーネント名]）を検証・修正してください。",
    "parameters": {
      "taskFile": "specs/stories/{NOTION_STORY_ID}-{title}/tasks/[タスクファイル名]",
      "component": "[コンポーネント名]",
      "implementationPath": "frontend/src/components/[コンポーネント名].tsx",
      "figmaCache": "specs/stories/{NOTION_STORY_ID}-{title}/design-cache/"
    }
  },
...
```json
{
  ...
  "nextAction": {
    ...
    "subagent_type": "visual-checker",
    "description": "ビジュアル検証",
    "prompt": "チェックリスト: frontend/tests/visual/[ID]-[機能名].visual-checklist.md の全項目をチェックしてください。Figmaキャッシュ: specs/stories/{NOTION_STORY_ID}-{title}/design-cache/ 実装ディレクトリ: frontend/src/components/, frontend/src/app/",
    "parameters": {
