...
- [x] task-transform-phase1-001: ExternalApiRecord型定義と単体テスト作成
- [x] task-transform-phase1-002: ITransformerインターフェースと関連型定義
- [x] task-transform-phase1-003: 日時ユーティリティ実装と単体テスト作成

## フェーズ完了確認

### 1. 成果物の存在確認

- [x] `src/types/external-api.ts` が存在する
- [x] `src/interfaces/transformer.ts` が存在する
- [x] `src/utils/date-utils.ts` が存在する
- [x] `test/unit/types/external-api.test.ts` が存在する
- [x] `test/unit/utils/date-utils.test.ts` が存在する
...
- [x] Phase 1の単体テストが全てパスすること
  ```bash
  npm test -- test/unit/types/external-api.test.ts test/unit/utils/date-utils.test.ts
  ```
