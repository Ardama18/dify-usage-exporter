...
## 対象ファイル
- [x] `src/types/env.ts` - 環境変数スキーマ拡張
- [x] `src/config/env-config.ts` - 更新（必要な場合）
- [x] `test/unit/types/env.test.ts` - 既存テストに新規項目追加

## 実装手順（TDD: Red-Green-Refactor）

### 1. Red Phase
- [x] 既存の `test/unit/types/env.test.ts` を確認
- [x] 新規環境変数のテストケースを追加
...
### 2. Green Phase
- [x] `src/types/env.ts` を読み込み、既存スキーマを確認
- [x] Dify Fetcher関連の環境変数を追加
  ```typescript
  // Dify Fetcher関連（新規追加）
...
- [x] `src/config/env-config.ts` が変更必要か確認（通常は不要）
- [x] テスト実行して通ることを確認
  ```bash
  npm test -- test/unit/types/env.test.ts
  ```
...
- [x] Biome lint: エラー0件
  ...
  npm run check
  ```
- [x] 新規環境変数がスキーマに追加されている
- [x] デフォルト値が正しく設定されている
- [x] 範囲チェック（min/max）が機能している
